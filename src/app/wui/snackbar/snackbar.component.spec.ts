...
import { FlSnackbarModule, FlSnackbar, FlSnackbarConfig, SimpleSnackbar } from './index';
...
describe('MdSnackBar', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [FlSnackbarModule, SnackBarTestModule, NoopAnimationsModule],
      providers: [
  ...
  beforeEach(inject([FlSnackbar, LiveAnnouncer], (sb: FlSnackbar, la: LiveAnnouncer) => {
    snackBar = sb;
    liveAnnouncer = la;
  }));
  ...
  it('should dismiss automatically after a specified timeout', fakeAsync(() => {
    let dismissObservableCompleted = false;
    let config = new FlSnackbarConfig();
    config.duration = 250;
...
describe('MdSnackBar with parent MdSnackBar', () => {
  let parentSnackBar: FlSnackbar;
  let childSnackBar: FlSnackbar;
  let overlayContainerElement: HTMLElement;
...
@NgModule({
  imports: [CommonModule, FlSnackbarModule],
